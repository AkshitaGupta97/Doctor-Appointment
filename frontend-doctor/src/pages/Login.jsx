  ...
  const onSubmitHandler = (event) => {
    ...
    try {
      ...
      if (state === 'Sign Up') {
        ...
        const { data } = axios.post(`${backendUrl}/api/user/login`, { email, password });
        if(data.success) {
          localStorage.setItem('token', data.token);
          setToken(data.token);
          toast.success(data.message);
...
return (
  <form onSubmit={onSubmitHandler} className="min-h-[80vh]  flex items-center">
    <div className={`flex flex-col gap-2 m-auto items-start ${state === 'Sign Up' ? "bg-teal-900" : "bg-blue-900"}  text-white p-8 md:p-5 min-w-85 sm:min-w-96 border border-amber-300 rounded-xl shadow-lg  `}>
      ...
      <button type='submit' className="mt-4 bg-gray-100 text-gray-800 w-full rounded-2xl px-1 py-2 hover:scale-95 hover:bg-gray-300 cursor-pointer transation-all">
